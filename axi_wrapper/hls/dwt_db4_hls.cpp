...
#pragma HLS INTERFACE s_axilite port = return
	...
	while (!all_done)
	{
		...
		for (int lvl = 0; lvl < DWT_LEVELS; lvl++)
		{
            ...
            if(!shift_reg[lvl].newdata)
                continue;

			if (shift_reg[lvl].count <= 7)
                continue;

			// to make symmetric at the beggining of stream
			if (shift_reg[lvl].count == 8)
			{
				shift_reg[lvl].make_symmetric_up();
				//do_conv = true;
			}
			// to make symmetric at the end of stream
			if (shift_reg[lvl].count > dwt_levels_size[lvl])
			{
				int index = (shift_reg[lvl].count - dwt_levels_size[lvl]) * 2 - 1;
